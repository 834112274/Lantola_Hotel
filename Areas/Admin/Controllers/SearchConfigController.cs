...
namespace HotelSystem.Web.Areas.Admin.Controllers
{
    public class SearchConfigController : Controller
    {
        ...
        {
            var t = from c in Query.db.HomeConfig
                     where c.Valid==true
                     select c;
            if (t.Count() > 0)
            {
        ...
        public ActionResult Save(HomeConfig config, HttpPostedFileBase ImgUrl)
        {
            ...
            if (ImgUrl != null && ImgUrl.ContentLength > 0)
            {
                ...
                config.ImgUrl = TimePath + Guid.NewGuid().ToString() + "." + FileType; //得到重命名的文件名
                ImgUrl.SaveAs(Server.MapPath(config.ImgUrl)); //保存操作
            }
            ...
            if (string.IsNullOrEmpty(config.Id))
            {
                ...
                Query.db.HomeConfig.Add(config);
            }
            else
            {
                var t = (from c in Query.db.HomeConfig
